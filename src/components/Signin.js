...
import { Button, Grid, TextField, sliderClasses } from '@mui/material';
    ...
    <div>
      <Grid container>
        <Grid item xs={6} sx={{ paddingLeft: '80px', paddingTop: '15px' }}>
          ...
          <img src={linkedin} style={{ width: '130px' }} />
          ...
          <TextField
            onChange={(e) =>
              setDesignation(e.target.value.charAt(0).toUpperCase() + e.target.value.slice(1))
            }
            size="small"
            variant="outlined"
            label="Designation"
        ...
        <Grid item xs={6}>
          <img style={{ width: '500px' }} src={develop} />
        </Grid>
      </Grid>
    </div>
  );
